 ...
 *
 * Note: The template Doc should not include a "Regular Meetings" heading —
 * each day heading (e.g. "Sunday Meetings") is generated by this script.
 */
function createGoogleDocFromSheet() {
  ...
  for (var day of daysOfWeek) {
    if (groups[day]) {
      // Print the day name with "Meetings" appended as a bold 10pt heading
      // e.g. "Sunday Meetings", "Monday Meetings", etc.
      var dayHeading = day + ' Meetings';
      var dayParagraph = newBody.appendParagraph(dayHeading);
      dayParagraph.setFontSize(10);
      dayParagraph.editAsText().setBold(0, dayHeading.length - 1, true);
  ...
  // setSpacingBefore(6) adds a small visual gap between the table and revision line.
  var currentDate = new Date();
  var formattedDate = Utilities.formatDate(currentDate, "GMT-07:00", "MM/dd/yyyy");
  ...
  if (lastElement.getType() === DocumentApp.ElementType.PARAGRAPH && lastElement.asText().getText() === '') {
    ...
    revisionParagraph.setFontSize(8);
    revisionParagraph.setSpacingBefore(6);
